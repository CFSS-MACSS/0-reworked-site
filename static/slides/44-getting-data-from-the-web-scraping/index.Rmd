...
```{r nodes, dependson = "get-statement", results='hide', cache = T}
dwight <- read_html(x = url)
# Extract anchor tags
out <- html_elements(x = dwight, css = "a") 
...
```{r nodes-out, dependson = "nodes", cache= TRUE}
out
```
...
*we can use a loop to go over filennames!*
...
#### Example 1: Scripting on prez statements

> Note: we will practice with SCRIPTS (instead of Rmd files)


#### Example 2: Sperling's Best Places 
