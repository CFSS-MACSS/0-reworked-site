...
* Layers: understanding ggplot
  * [revisit prior slides if needed!](https://cfss-macss.netlify.app/slides/12-visualizations-and-the-grammar-of-graphics/#1)
* Intro to Viz
  * aesthetics (aes)
...
# Plotting basics: Layering our cake


---
##   aesthetics (aes)

Aesthetic mappings are part of our fundamental layer of our plots! They tell us the THING that is going to be mapped from our DATA. Anything that goes inside `aes()` will vary based on the data. If we put something in our ggplot layer (or geom) that is OUTSIDE the `aes()` argument, it will be constant (NOT vary). 

---
### aes examples:
.panelset[
  .panel[.panel-name[Opt 1]
.pull-left[
```{r mpg-1, fig.show='hide', echo = T}
...
![](`r knitr::fig_chunk("mpg-1", ".png")`)

]]
.panel[.panel-name[Opt 2]
.pull-left[
```{r mpg-2, fig.show='hide', echo = T}
ggplot(mpg) +
...
.pull-right[
![](`r knitr::fig_chunk("mpg-2", ".png")`)
  ]
  ]


.panel[.panel-name[Opt 3]
.pull-left[
```{r mpg-3, fig.show='hide', echo = T}
ggplot(mpg) +
  geom_point(aes(x = displ, y = hwy), shape = 4)
```
]

.pull-right[
![](`r knitr::fig_chunk("mpg-3", ".png")`)
]
]
]
...
* Color (by variable or setting)
* Transparency/opacity (by variable or setting)
* Size (by variable or setting)
* Shape (by variable or setting)


---
##    geometric objects (geoms)
Geoms are the way we represent our data -- specifically, how it is that we want to add/represent our data layer
...
![](`r knitr::fig_chunk("mpg-4", ".png")`)

]]
.panel[.panel-name[Opt 2]
.pull-left[
```{r mpg-5, fig.show='hide', echo = T}
ggplot(mpg, aes(x = hwy)) +
  geom_density()
```
]
...
We use facets to separate plots in a meaningful way -- for example, if we want to do some sort of categorical breakout. The main options are

- No facets (default and what we've been working with)
- `facet_wrap`: think of this as breaking out by one categorical variable and setting how many sub-plots per row
- `facet_grid`: more sophsiticated and allows breakouts by TWO (or more) variables
...
.panelset[
  .panel[.panel-name[Base level]
.pull-left[
```{r mpg-7, fig.show='hide', echo = T}
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(binwidth = 2) 
```
]
...
![](`r knitr::fig_chunk("mpg-7", ".png")`)

]]
.panel[.panel-name[Wrap]
.pull-left[
```{r mpg-8, fig.show='hide', echo = T}
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(binwidth = 2) + 
  facet_wrap(vars(cyl))
```
...
.pull-right[
![](`r knitr::fig_chunk("mpg-8", ".png")`)
  ]
  ]


.panel[.panel-name[Grid A]
.pull-left[
```{r mpg-9, fig.show='hide', echo = T}
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(binwidth = 2) + 
  facet_grid(~cyl)
```
...
.pull-right[
![](`r knitr::fig_chunk("mpg-9", ".png")`)
]
]


.panel[.panel-name[Grid B]
.pull-left[
```{r mpg-10, fig.show='hide', echo = T}
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(binwidth = 2) + 
  facet_grid(drv~cyl)
```
...
---
##   stats transformations
You can conduct additional calculations on data to display summary statistics. You can either calculate this yourself OR you can do calculations on the data during plotting. (it depends on if/whether you need the results)

* `stat = identity` (inside `geom_bar`)
* `after_stat` [see more here!](https://ggplot2.tidyverse.org/reference/aes_eval.html)
* `stat_summary` (I use this the most!)

---
## Transformation examples

.panelset[
  .panel[.panel-name[Bar count]
.pull-left[
```{r mpg-11, fig.show='hide', echo = T}
ggplot(mpg, aes(x = hwy)) +
  geom_bar(stat = 'count' )
```
]
...
![](`r knitr::fig_chunk("mpg-11", ".png")`)

]]
.panel[.panel-name[Bar identity]
.pull-left[
```{r mpg-12, fig.show='hide', echo = T}
...
.pull-right[
![](`r knitr::fig_chunk("mpg-12", ".png")`)
  ]
  ]


.panel[.panel-name[After Stat]
.pull-left[
```{r mpg-13, fig.show='hide', echo = T}
ggplot(mpg, aes(x = hwy, y = after_stat(prop))) +
  geom_bar()
```
]

.pull-right[
![](`r knitr::fig_chunk("mpg-13", ".png")`)
]
]

.panel[.panel-name[Stat Sum]
.pull-left[
```{r mpg-14, fig.show='hide', echo = T}
ggplot(mpg) +
    stat_summary(
        aes(x = hwy, y = cty),
        fun.min = min,
...
---
##  position adjustments
Position has to do with the placment of bars: next to, stacked, or scaled to fill to a certain height. *Note: there is position = "fill" and fill = `var` that are different!*
...
![](`r knitr::fig_chunk("mpg-15", ".png")`)

]]
.panel[.panel-name[Stack]
.pull-left[
```{r mpg-16, fig.show='hide', echo = T}
ggplot(mpg, aes(x = drv, fill = class)) + 
...
We're not going to get into this too much for now, know that you can map things and use polar coordinates. If it becomes relevant for your project, we can talk more and/or you can take Data Viz!

---

## Recap:

* Different layers to our cake -- start with ggplot and build from there:
  * aesthetics (aes)
  * geometric objects (geoms)
  * facets
  * stats transformations
...
**All of these can be part of initial investigations in order to get a sense of your data and generate questions**: 
...
Chapter 10 of R for Data Science defines EDA as an iterative process:
...
```{r scatterplot-mult-channels2, include = T, echo = T, warning = F}
ggplot(data = scorecard,
  aes(  x = cost, y = netcost, color = type )) +
  geom_point() + scale_color_brewer(palette = "Dark2")
```

[ggplot themes](https://ggplot2.tidyverse.org/reference/ggtheme.html) and [color brewer](https://ggplot2.tidyverse.org/reference/scale_brewer.html), and [colorblind friendly](https://jrnold.github.io/ggthemes/reference/colorblind.html)
---
...
[**Chapter 16 of R for Data Science**](https://r4ds.had.co.nz/factors.html) goes in-depth on creating and modifying factors: 
...
Use `data()` to find a suitable R dataset to explore
* make a histogram
* make a bar plot with a fill
  * stack it
  * dodge it
  * position = fill it
