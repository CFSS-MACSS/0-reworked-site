...
```{r, eval = F}
alice %>% str_remove_all("\\.") %>% str_replace_all("[A-Z]", function(x) tolower(x))
```
...
* option 3: `install.packages("styler")` (should be able to find it under addins at top right)
...
`usethis::use_course("CFSS-MACSS/debugging")`
...
* [(optional) Register on geonames](https://www.geonames.org/export/web-services.html)
