...
author: "[MACS 30500](https://cfss-macss.netlify.app) <br /> University of Chicago"
output:
...
We have data about `r nrow(yob2023)` names for Females born in 2023 . We find that `r nrow(yob2023) - nrow(fem)` are males. 

```{r youth-dist, echo = FALSE}
fem %>% mutate(nrow = sort(desc(Num_Birth)))
  ggplot(aes(age)) + 
...
```bash
remote: Permission to hadley/ggplot2.git denied to jmclip.
