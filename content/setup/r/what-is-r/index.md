...
* Add-on libraries - previously Python was criticized for its lack of libraries to perform statistical analysis and data manipulation, especially relative to the plethora of libraries for R. In recent years Python has begun to catch up with libraries for scientific computing ([`numpy`](http://www.numpy.org/)), data analysis ([`pandas`](http://pandas.pydata.org/)), and machine learning ([`scikit-learn`](http://scikit-learn.org/stable/)).The biggest hurdle is managing the virtual environments-- although there are package managers such as [`conda`](https://conda.io/docs/) and [uv](https://docs.astral.sh/uv/) that can help.
